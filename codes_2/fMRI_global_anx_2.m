%This code computes the analysis presented in Kundert-Obando et al., 2025
...
ica_mask = niftiread("melodic_IC.nii.gz");
...
arousal_template = niftiread('full_template.nii'); %can be downloaded at 
...
subj= readtable('great_HR_sub_coded.txt', 'readvariablenames', 0);


% where to save the outputs for each subject
%save_path = 'subject_outputs';
...
for subject = 1:height(subj)
    ...
    betas_r2 = pinv(betas_r1_ones)*Y_scaled;
    ...
    global_signal_tc= mean(Y_scaled'); 
    ...
    Y_new=100*(Y_hm_removed - mean(Y_scaled,1)) ./ mean(Y_scaled,1);

    %zscoring Y_new
    Y_zscore= zscore(Y_scaled); %Check to be zscoring across time dimension as rows
...
%% create arousal signal time course -> arousal_signal
    ...
    V_zscored = zscore(Y_scaled)'; % output is (vox x time)
    ...
    if subj.Var2(subject) == 1 %a 1 here indicates that this subject had clean HR
        ...
        betas_hr = pinv(X1_hr) * Y_scaled; 
        hr_confound = X1_hr(:,2:end) * betas_hr(2:end,:);
        Y_hr = Y_scaled - hr_confound; % this will be a new fMRI with HR removed
        hr_percent_variance_map = var(hr_confound) ./  var(Y_scaled);
...
write_output_sheet(append(save_path,'/','regressed_A.csv'), network_labels, regA_allsub, subject_IDs);
write_output_sheet(append(save_dir,'/','regressed_G.csv'), network_labels, regG_allsub, subject_IDs);
write_output_sheet(append(save_dir,'/','regressed_H.csv'), network_labels, regH_allsub, subject_IDs);
write_output_sheet(append(save_dir,'/','regressed_All.csv'), network_labels, regAll_allsub, subject_IDs);
write_output_sheet(append(save_dir,'/','regressed_AG.csv'), network_labels, regAG_allsub, subject_IDs);
write_output_sheet(append(save_dir,'/','non_regressed.csv'), network_labels, noreg_allsub, subject_IDs);
...
T_path=[save_path,'/','Networks_Corr_Global_Components_last_mo_corr.csv']
...
T_path2=[save_path,'/','GlobalComponents_Corr_GlobalComponents_mo_corr.csv']
